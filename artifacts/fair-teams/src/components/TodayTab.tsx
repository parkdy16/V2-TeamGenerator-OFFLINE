    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between bg-primary text-primary-foreground p-3 rounded-xl shadow-sm">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="secondary" size="sm" onClick={() => setPlayers(players.map(p => ({ ...p, attending: false })))} className="h-7 text-[10px] font-bold uppercase px-2 opacity-80 hover:opacity-100 bg-black/10 hover:bg-black/20 text-white border-transparent" >
            Clear
          </Button>
        </div>
      </div>
          ...
          {filtered.map(player => (
            ...
            >
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <span className={`font-bold text-xs truncate leading-tight ${player.attending ? "text-primary" : "text-foreground"}`}>{player.name}</span>
                <span className="text-[10px] text-muted-foreground font-medium">Skill {player.skill}</span>
              </div>
            </label>
          ))}
        </div>
